...
def simple_dense_net(inp,layers,act = tf.nn.relu,out_act = "default",dropout = 1):
    ...
    for l in layers[:-1]:
        temp = tf.nn.dense(temp,l,activation = act)
        if dropout < 1:
            temp = tf.nn.dropout(temp,dropout)

    return tf.nn.dense(temp,layers[-1],activation = m_outact)
